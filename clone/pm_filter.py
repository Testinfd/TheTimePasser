    ...
    btn = [
        [
            InlineKeyboardButton(
                text=f"[{get_size(file_item['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file_item['file_name'].split()))}", callback_data=f"file#{file_item['file_id']}"
            ),
        ]
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn = [
        [
            InlineKeyboardButton(
                text=f"⚜️[{get_size(file_item['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file_item['file_name'].split()))}", callback_data=f"file#{file_item['file_id']}"
            ),
        ]
        for file_item in files
    ] 
...
async def advantage_spell_chok(client, name, msg, reply_msg, ai_search):
    ...
    if ai_search == True:
        ai_search_new = False
        ai_msg = await reply_msg.edit_text("<b><i>Advanced AI Trying To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
        movienamelist += [movie.get('title') for movie in movies]
        for movie_item in movienamelist:
            try:
                mv_rqst = mv_rqst.capitalize()
            except:
                pass
            if mv_rqst.startswith(movie_item[0]):
                await auto_filter(client, movie_item, msg, reply_msg, ai_search_new)
                break
        reqst_gle = mv_rqst.replace(" ", "+")
        button = [[
            InlineKeyboardButton("Gᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
